...
function New-LabVM
{
  ...
  Process
  {
    ...
    if ($DSC -eq $true)
    {
      ...
      $destmof = "F:\Windows\system32\Configuration\MetaConfig.mof"
      Move-Item -Path $sourcemof -Destination $destmof -Force

      Write-Verbose -Message "Copied metaconfig.mof to F:\Windows\system32\Configuration\MetaConfig.mof"
      
      reg.exe load HKLM\Vhd F:\Windows\System32\Config\Software
...
#How to use, u write the module with parameters
#New-LabVM -VMName test -GWIP 10.11.12.1 -VMIP 10.11.12.2 
